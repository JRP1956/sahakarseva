...
export default function ForecastBars({ rows }: { rows: FRow[] }) {
  ...
  return (
    <div className="space-y-2">
      {rows.map((r, i) => (
        <div key={i} className="grid grid-cols-[6rem_1fr_6rem] sm:grid-cols-[8rem_1fr_7rem] items-center gap-2 sm:gap-3 text-sm">
          ...
          <span className="text-right tnum whitespace-nowrap">{r.predicted.toFixed(0)} <span className="text-fg-2">/ {r.available_workers}</span>{r.shortage > 0 && <span className="text-error-fg font-medium ml-1">short {r.shortage}</span>}</span>
        </div>
      ))}
    </div>
  );
